...
- [Past Events](https://info.corenet.gov.sg/resources/past--events) → `snapshot.txt`
- [Circulars](https://info.corenet.gov.sg/resources/circulars) → `snapshot-circulars.txt`
...
2. Compares it to the page's snapshot file (the content saved from the previous run).
3. If the content differs, sends a push notification summarizing what changed, and commits the updated content to that snapshot file with a descriptive commit message.
4. If nothing changed for a page, does nothing further for it (no commit).
...
1. `GET https://info.corenet.gov.sg/api/token/init` with browser-like headers (`User-Agent`, `Referer: https://info.corenet.gov.sg/resources/circulars`, `Origin: https://info.corenet.gov.sg`, `Accept: application/json, text/plain, */*`) — requests without these headers get a `403 Access denied`. Returns `{"accessToken": "..."}` (a short-lived JWT).
2. `POST https://info.corenet.gov.sg/api/Circulars/list` with `Authorization: Bearer <accessToken>` and the same browser-like headers, body:
   ```json
